...
def stop_hunt_metrics(df: pl.DataFrame, lookback: int = 20) -> pl.DataFrame:
    ...
    df = df.with_columns([
        ...
        (pl.col("upper_wick") / (pl.col("range").rolling_mean(window_size=lookback) + 1e-8)).alias("upper_wick_rel"),
        (pl.col("lower_wick") / (pl.col("range").rolling_mean(window_size=lookback) + 1e-8)).alias("lower_wick_rel"),
    ])

    # Calculate z-scores for anomaly detection
    df = df.with_columns([
        # Z-scores for wick sizes
        ((pl.col("upper_wick") - pl.col("upper_wick").rolling_mean(window_size=lookback)) /
         (pl.col("upper_wick").rolling_std(window_size=lookback) + 1e-8)).alias("upper_wick_z"),

        ((pl.col("lower_wick") - pl.col("lower_wick").rolling_mean(window_size=lookback)) /
         (pl.col("lower_wick").rolling_std(window_size=lookback) + 1e-8)).alias("lower_wick_z"),

        # Volume spike detection
        ((pl.col("volume") - pl.col("volume").rolling_mean(window_size=lookback)) /
         (pl.col("volume").rolling_std(window_size=lookback) + 1e-8)).alias("volume_z"),
    ])
    ...
    df = df.with_columns([
        ...
        (pl.col("high") > pl.col("high").rolling_max(window_size=lookback).shift(1)).alias("high_sweep"),
        (pl.col("low") < pl.col("low").rolling_min(window_size=lookback).shift(1)).alias("low_sweep"),
    ])
