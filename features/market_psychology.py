...
def add_market_psychology_features(df: pl.DataFrame, lookback: int = 20) -> pl.DataFrame:
    ...
    df = df.with_columns([
        ...
        ((pl.col("close") - pl.col("low").rolling_min(lookback)) / 
         (pl.col("high").rolling_max(lookback) - pl.col("low").rolling_min(lookback) + 1e-8))
    ...
    df = df.with_columns([
        ...
        ((pl.col("close") < pl.col("open")) & 
         (pl.col("volume") > pl.col("volume").rolling_mean(lookback) * 1.5) &
    ...
    df = df.with_columns([
        pl.col("fear_spike").cast(pl.Int32).rolling_sum(lookback).alias("fear_index"),
        pl.col("greed_spike").cast(pl.Int32).rolling_sum(lookback).alias("greed_index"),
    ])
    ...
    df = df.with_columns([
        ...
        (pl.col("greed_index") - pl.col("fear_index")).rolling_std(lookback)
    ...
    df = df.with_columns([
        ...
        ((pl.col("fear_index") > lookback * 0.7) & 
         ...
         (pl.col("volume") > pl.col("volume").rolling_max(lookback*2) * 0.8))
    ...
    df = df.with_columns([
        ...
        (pl.col("close").rolling_std(lookback) / 
         (pl.col("close").rolling_mean(lookback) + 1e-8)).alias("price_dispersion"),
        
        # Volume consensus (everyone trading)
        (pl.col("volume") / pl.col("volume").rolling_mean(lookback*3))
    ...
    df = df.with_columns([
        ...
        ((pl.col("close").rolling_std(5) < pl.col("close").rolling_std(lookback) * 0.5) &
         (pl.col("close").diff(5).abs() > pl.col("close").rolling_std(lookback) * 2))
        .alias("regret_pattern"),
        
        # Relief: price stabilizes after volatile period
        ((pl.col("close").rolling_std(5) < pl.col("close").rolling_std(lookback) * 0.5) &
         (pl.col("close").shift(5).rolling_std(5) > pl.col("close").rolling_std(lookback) * 1.5))
    ...
    df = df.with_columns([
        ...
        ((pl.col("close") - pl.col("high").rolling_max(lookback)).abs() < 
         pl.col("close") * 0.001).rolling_sum(lookback).alias("high_anchoring"),
        
        ((pl.col("close") - pl.col("low").rolling_min(lookback)).abs() < 
         pl.col("close") * 0.001).rolling_sum(lookback).alias("low_anchoring"),
    ])
    ...
    df = df.with_columns([
        ...
        .rolling_sum(lookback).alias("decision_fatigue"),
    ])
    
    # Loss aversion patterns
    df = df.with_columns([
        # Holding losers: low volume during drawdown
        ((pl.col("close") < pl.col("close").rolling_max(lookback) * 0.95) &
         (pl.col("volume") < pl.col("volume").rolling_mean(lookback) * 0.8))
        .alias("loss_aversion"),
        
        # Profit taking: volume spike near resistance
        ((pl.col("close") > pl.col("close").rolling_max(lookback) * 0.98) &
         (pl.col("volume") > pl.col("volume").rolling_mean(lookback) * 1.2))
        .alias("profit_taking"),
    ])
