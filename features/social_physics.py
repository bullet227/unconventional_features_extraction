...
def add_social_physics_features(df: pl.DataFrame, lookback: int = 20) -> pl.DataFrame:
    ...
    df = df.with_columns([
        ...
        pl.col("price_velocity").rolling_std(window_size=lookback).alias("market_temperature"),

        # Entropy (disorder) based on price dispersion
        (pl.col("high") - pl.col("low")).rolling_std(window_size=lookback).alias("market_entropy"),
    ])
    
    # Pressure features (volume concentration)
    df = df.with_columns([
        # Volume pressure (volume relative to average)
        (pl.col("volume") / pl.col("volume").rolling_mean(window_size=lookback)).alias("volume_pressure"),
    ...
    df = df.with_columns([
        ...
        (pl.col("close") - pl.col("close").rolling_mean(window_size=lookback)).alias("ma_gravity"),
        (pl.col("close") - pl.col("close").rolling_mean(window_size=lookback*2)).alias("ma_gravity_long"),

        # Elastic force (mean reversion strength)
        ((pl.col("close") - pl.col("close").rolling_mean(window_size=lookback)) /
         pl.col("close").rolling_std(window_size=lookback)).alias("elastic_force"),
    ])
    ...
    df = df.with_columns([
        ...
        ((pl.col("market_temperature") > pl.col("market_temperature").rolling_mean(window_size=lookback*2) * 1.5) |
         (pl.col("market_entropy") > pl.col("market_entropy").rolling_mean(window_size=lookback*2) * 1.5)).alias("phase_transition"),
    ...
    df = df.with_columns([
        # Price-volume correlation as coherence measure (rolling correlation)
        (((pl.col("price_velocity") - pl.col("price_velocity").rolling_mean(window_size=lookback)) *
          (pl.col("market_mass") - pl.col("market_mass").rolling_mean(window_size=lookback)))
         .rolling_mean(window_size=lookback) /
         (pl.col("price_velocity").rolling_std(window_size=lookback) *
          pl.col("market_mass").rolling_std(window_size=lookback) + 1e-8))
        .alias("crowd_coherence"),

        # Synchronization index (how aligned are market forces)
        (pl.col("market_force").rolling_std(window_size=lookback) /
         (pl.col("market_force").rolling_mean(window_size=lookback).abs() + 1e-8)).alias("force_synchronization"),
    ])
